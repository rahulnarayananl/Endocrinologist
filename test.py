...
def login():
    ...
    doc=ref.get()
    output_ref = db.collection(u'Users').document(u'#name')
    a=doc.to_dict()
    ...
    if output==0:
        output_ref.update({u'Output': '0'})
        return render_template('form2false.html')
    else:
        output_ref.update({u'Output': '1'})
        return render_template('form2true.html')
